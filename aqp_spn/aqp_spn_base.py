...
class AQPSPN(CombineSPN, RSPN):
    ...
    def learn_uniform(self, train_data, sum_fanout=8, product_fanout=4, max_depth=12):
        ...
        self.mspn = self._build_constrained_node(train_data, scope, 0, sum_fanout, product_fanout, max_depth, data_id)
        assign_ids(self.mspn)

    def _build_constrained_node(self, data, scope, depth, sum_fanout, product_fanout, max_depth, data_id):
        ...
        # Base case: Leaf
        if len(scope) == 1:
            return create_custom_leaf(data, self.ds_context, scope, data_id)

        # Product Node (Odd depth or Max Depth reached)
        if (depth % 2 != 0) or (depth >= max_depth):
            # Split scope
            if depth >= max_depth:
                # Force full factorization if max depth reached
                num_chunks = len(scope)
            else:
                num_chunks = min(len(scope), product_fanout)
            
            chunk_size = len(scope) // num_chunks
            remainder = len(scope) % num_chunks

            children = []
            start = 0
            for i in range(num_chunks):
                end = start + chunk_size + (1 if i < remainder else 0)
                sub_scope = scope[start:end]
                
                # Slice data: columns corresponding to sub_scope
                # Since data columns correspond to scope, we just take columns start:end
                sub_data = data[:, start:end]
                
                child = self._build_constrained_node(sub_data, sub_scope, depth + 1, sum_fanout, product_fanout, max_depth, data_id)
        ...
        # Sum Node (Even depth)
            ...
            try:
                ...
                # Fallback: treat as single cluster
            ...
            for i in range(len(result)):
                ...
                child = self._build_constrained_node(sub_data, sub_scope, depth + 1, sum_fanout, product_fanout, max_depth, sub_data_id)
                children.append(child)
