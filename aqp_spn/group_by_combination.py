...
def group_by_combinations(spn, ds_context, feature_scope, ranges, node_distinct_vals=None, node_likelihoods=None):
    ...
    logger.info(
        "[QUERY TRACE][group_by_combinations] feature_scope=%s evidence_scope=%s relevant_scope=%s evidence_shape=%s",
        feature_scope_unsorted,
    ...
    if isinstance(result, tuple):
        _, grouped_tuples = result
        result_count = 0 if grouped_tuples is None else len(grouped_tuples)
        sample_tuples = [] if grouped_tuples is None else list(grouped_tuples)[:5]
        logger.info(
            "[QUERY TRACE][group_by_combinations] grouped_tuple_count=%d sample_grouped_tuples=%s",
            result_count,
            sample_tuples,
        )
    # print('result: ', result)
