...
class SPNEnsemble:
    ...
    def _evaluate_group_by_spn_ensembles(self, query):
        ...
        logger.info("[QUERY TRACE][group_by_dispatch] requested_group_bys=%s", group_by_list)
        for spn, attribute_list in dict_items:
            logger.info(
                "[QUERY TRACE][group_by_dispatch] spn_tables=%s assigned_group_bys=%s",
                sorted(list(spn.table_set)) if spn is not None else None,
                attribute_list,
            )
        ...
        logger.info(
            "[QUERY TRACE][group_by_dispatch] group_scopes=%s total_group_candidates=%d sample_groups=%s",
            group_bys_scopes,
            len(result_tuples_translated),
            result_tuples_translated[:5],
        )
    ...
    def evaluate_query(self, query, rdc_spn_selection=False, pairwise_rdc_path=None,
        ...
        logger.info(
            "[QUERY TRACE][evaluate_query] query_type=%s tables=%s joins=%s group_bys=%s aggregations=%s where=%s",
            query.query_type.name,
            sorted(query.table_set),
            sorted(query.relationship_set),
            query.group_bys,
            query.aggregation_operations,
            query.table_where_condition_dict,
        )
        ...
        if len(query.group_bys) > 0:
            ...
            if not result_tuples_translated:
                ...
                logger.info(
                    "[QUERY TRACE][evaluate_query] technical_group_by_scopes=%s sample_result_tuples=%s",
                    technical_group_by_scopes,
                    result_tuples_translated[:5],
                )
                if debug:
        ...
        if query.query_type == QueryType.CARDINALITY or any(
            ...
            logger.info(
                "[QUERY TRACE][evaluate_query] artificially_added_conditions=%s",
                artificially_added_conditions,
            )
            ...
            logger.info(
                "[QUERY TRACE][evaluate_query] factors_count=%d cardinalities_shape=%s",
                len(factors),
                getattr(cardinalities, 'shape', None),
            )
            if debug:
                if len(query.group_bys) == 0:
        ...
        if all_operations_of_type(AggregationType.SUM, query) or all_operations_of_type(AggregationType.AVG, query):
            ...
            for aggregation_operation_type, aggregation_type, factors in query.aggregation_operations:
                ...
                # which aggregation
                    ...
                    logger.info(
                        "[QUERY TRACE][evaluate_query] expectation_spn_tables=%s expectation=%s",
                        sorted(expectation_spn.table_set),
                        expectation,
                    )
                    if confidence_intervals:
    ...
    def cardinality(self, query, rdc_spn_selection=False, pairwise_rdc_path=None,
        ...
        # 根据返回结果里的cardinality进行排序，取中位数返回
