...
def compute_ground_truth(target_path, physical_db_name, vacuum=False, query_filename=None, query_list=None):
    ...
    for query_no, query_str in enumerate(queries):
        ...
        logger.info(f"\033[93m{ground_truth[query_no]}\033[0m")  # 黄色
...
def compute_relative_error(true, predicted, debug=False):
    ...
    if true == 0.0:
        relative_error = 0.0 if predicted == 0.0 else 1.0
    else:
        ...
        try:
            ...
            if ground_truth is not None and ground_truth[query_no]:
                ...
                if isinstance(aqp_result, list):
                    ...
                    print(f"aqp_result[0]: {aqp_result[0]}\ntrue_result[0]: {true_result[0]}")
...
# 出现这种情况，是因为ground_truth的结果是0，而SPN预测的结果不是0
                    ...
                    logger.info(f"\t\tCalculation: |{true_result} - {predicted_value}| / {true_result} = {relative_error}")
    ...
    # 输出平均水平和中位数水平
    if len(csv_rows) > 0:
        latencies = [row['latency'] for row in csv_rows]
        # Filter out error codes (-3 for SPN fail)
        errors = [row['average_relative_error'] for row in csv_rows if row['average_relative_error'] != -3]
        
        avg_latency = sum(latencies) / len(latencies)
        avg_error = sum(errors) / len(errors) if len(errors) > 0 else 0
        median_error = np.median(errors) if len(errors) > 0 else 0
        ...
        logger.info(f"==========================================")
        logger.info(f"Total Queries Evaluated: {GREEN}{len(csv_rows)}{RESET}")
        logger.info(f"Average Latency: {GREEN}{avg_latency:.4f}{RESET} secs")
        logger.info(f"Average Relative Error: {GREEN}{avg_error * 100:.2f}%{RESET}")
        logger.info(f"Median Relative Error: {GREEN}{median_error * 100:.2f}%{RESET}")
        logger.info(f"==========================================")
