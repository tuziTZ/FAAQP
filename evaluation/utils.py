...
def parse_query(query_str, schema):
    ...
    def _log_query_trace(parsed_query):
        logger.info("[QUERY TRACE][parse_query] sql=%s", query_str)
        logger.info(
            "[QUERY TRACE][parse_query] query_type=%s tables=%s joins=%s group_bys=%s aggregations=%s",
            parsed_query.query_type.name,
            sorted(parsed_query.table_set),
            sorted(parsed_query.relationship_set),
            parsed_query.group_bys,
            parsed_query.aggregation_operations,
        )
        logger.info("[QUERY TRACE][parse_query] where=%s", parsed_query.table_where_condition_dict)
