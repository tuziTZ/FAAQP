...
from rspn.learning.rspn_learning import get_RDC_value
    ...
    def next_operation(
        ...
        if minimalInstances or (no_clusters and no_independencies):
            if multivariate_leaf:
                ...
                # print('structure_learning.py 85 222222222222222222222:', minimalInstances, no_clusters, no_independencies)
                return Operation.NAIVE_FACTORIZATION, None
...
def learn_structure(
    ...
    dataset_id = np.arange(root.cardinality)
    ...
    tasks = deque()
    tasks.append((dataset, root, 0, initial_scope, False, False, dataset_id, rdc_adjacency_matrix_dict))

    while tasks:

        local_data, parent, children_pos, scope, no_clusters, no_independencies, local_data_id, rdc_adjacency_matrix_dict = tasks.popleft()
        ...
        if operation == Operation.REMOVE_UNINFORMATIVE_FEATURES:
            ...
            for col in op_params:
                ...
                tasks.append(
                    (
                        data_slicer(local_data, [col], num_conditional_cols), #按列切分数据
                        node,
                        len(node.children) - 1,
                        [scope[col]],
                        True,
            ...
            data_slices, cluster_centers, data_slices_ids = split_rows(local_data, ds_context, scope, local_data_id)
            ...
            if len(data_slices) == 1:
                tasks.append((local_data, parent, children_pos, scope, True, False, local_data_id, rdc_adjacency_matrix_dict))
                continue

            create_sum_node(children_pos, data_slices, parent, scope, tasks, cluster_centers, data_slices_ids, rdc_adjacency_matrix_dict=rdc_adjacency_matrix_dict)

            continue

        elif operation == Operation.SPLIT_COLUMNS:
            split_start_t = perf_counter()
            data_slices, rdc_adjacency_matrix_dict = split_cols(local_data, ds_context, scope)
            split_end_t = perf_counter()
            logging.debug(
            ...
            if len(data_slices) == 1:
                tasks.append((local_data, parent, children_pos, scope, False, True, local_data_id, rdc_adjacency_matrix_dict))
            ...
            for data_slice, scope_slice, _ in data_slices:
                ...
                node.children.append(None)
                tasks.append((data_slice, node, len(node.children) - 1, scope_slice, False, False, local_data_id, rdc_adjacency_matrix_dict))
            ...
            split_start_t = perf_counter()
            # rdc_adjacency_matrix_dict = get_RDC_value(local_data, ds_context, scope)
            rdc_adjacency_matrix_dict = None
            for col in range(len(scope)):
                ...
                local_children_params.append((child_data_slice, ds_context, [scope[col]], local_data_id, rdc_adjacency_matrix_dict))
            ...
            node = create_leaf(local_data, ds_context, scope, local_data_id, rdc_adjacency_matrix_dict)
            parent.children[children_pos] = node
...
def create_sum_node(children_pos, data_slices, parent, scope, tasks, cluster_centers=None, data_slices_ids=None, rdc_adjacency_matrix_dict=None):
    ...
    cardinality = 0
    # print('structure_learning.py', len(data_slices), len(data_slices_ids))
    for i in range(len(data_slices)):
        data_slice, scope_slice, proportion = data_slices[i][0], data_slices[i][1], data_slices[i][2]
    # for data_slice, scope_slice, proportion in data_slices:
        assert isinstance(scope_slice, list), "slice must be a list"
        cardinality += len(data_slice)
        node.children.append(None)
        node.weights.append(proportion)
        tasks.append((data_slice, node, len(node.children) - 1, scope, False, False, data_slices_ids[i], rdc_adjacency_matrix_dict))
