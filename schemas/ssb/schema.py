...
def gen_sf50_ssb_schema(csv_path):
    ...
    # model1: 300527
    table_sizes = {'lineorder': 6001173,
                   'dwdate': 2557,
                   'part': 200000,
                   'supplier': 2000,
                   'customer': 30000}
    schema.table_dictionary['lineorder'].sample_rate = 1
    for table in schema.tables:
        table.table_size = table_sizes[table.table_name]
        schema.table_dictionary[table.table_name].table_size = table_sizes[table.table_name]
...
def gen_500gb_ssb_schema(csv_path):
    ...
    schema.add_table(Table('lineorder',
                           ...
                           # csv_file_location=csv_path.format('lineorder_sampled_001'),
                           csv_file_location=csv_path.format('lineorder'),
                           irrelevant_attributes=['lo_commitdate'],
                           table_size=3000028242, primary_key=['lo_orderkey', 'lo_linenumber'], sample_rate=0.0033))
